...
import cv2
import gata6_model_MesoEndo_V5 as RD
import sys
...
class GATA6_Adhesion_Coupled_Simulation(Simulation):
    ...
    def __init__(self, model_params):
        ...
        # aba/dox/cho ratio
        self.nanog_color = np.array([195, 186, 242], dtype=int) #purple
        self.gata6_color = np.array([50, 50, 255], dtype=int) #blue
    ...
    def setup(self):
        ...
        self.nanog_mrna = np.random.normal(6.5, size=self.number_agents)
        self.NANOG_prot = np.random.normal(6.5, size=self.number_agents)
        self.copy_number = self.agent_array(dtype=float)
        self.copy_number = self.copy_number = np.random.poisson(lam=4.3, size=self.number_agents) + 2
        ...
        for name in self.array_names:
            ...
            if 'conc' in name:
                for i in range(num_added):
                    ...
                    self.__dict__[name][mother] = self.__dict__[name][mother]
    ...
    def cell_fate(self):
        ...
        """
        Currently using rules related to high expression level I believe..
        """
        for i in range(self.number_agents):
            if self.NANOG_prot[i] > self.nanog_threshold:
                self.cell_type[i] = 0
            else:
                if self.FOXA2_prot[i] + self.FOXF1_prot[i] > self.bifurcation_threshold:
                    if self.FOXA2_prot[i] > self.FOXF1_prot[i]:
                        self.cell_type[i] = 2
                    elif self.FOXA2_prot[i] < self.FOXF1_prot[i]:
                        self.cell_type[i] = 3
                    else:
                        self.cell_type[i] = 1
                else:   
        ...
        if self.replication_type == 'Contact_Inhibition':
            ...
            for index in range(self.number_agents):
                if self.division_set[index] > self.div_thresh[index]:
                    ...
                    if np.sum(adjacency_matrix[index,:]) < 8:
